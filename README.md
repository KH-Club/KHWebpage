...
The `/news-activities` page reads published rows from the Supabase `events`
table through `src/services/newsActivityService.ts`. Public records support a
title, description, event date, registration start/end dates, countdown label,
location, image URL, category, status, detail route, action button, and publish
state. Content and image uploads are managed in `kaihor-backoffice` at
`/events`; the required SQL and Storage contract is in that repo at
`docs/news-activities.md`.

The large feature panel uses the newest created published record for the active
filter. If no published CMS records exist, the page shows the normal empty
state.
...
- **News & Activities** - Published camp updates with upcoming, completed, and announcement filters
